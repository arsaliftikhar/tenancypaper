...
import { Download, Loader2, FileText, ArrowRight, ArrowLeft, CheckCircle2, Camera, X, Pencil } from "lucide-react";
...
export default function GenerateWizard() {
  ...
  const generatePDF = async () => {
    try {
      ...
      // Left Col: X=180
      page1.drawText(formData.ownerName || formData.landlordName || '', { x: 300, y: 665, size, color });
      page1.drawText(formData.landlordName || '', { x: 300, y: 643, size, color });
      ...
      page1.drawText(formData.tenantName || '', { x: 300, y: 496, size, color });
      page1.drawText(formData.tenantId || '', { x: 100, y: 488, size, color });
      page1.drawText(formData.tenantEmail || '', { x: 100, y: 426, size, color });
      page1.drawText(formData.tenantPhone || '', { x: 100, y: 403, size, color });

      // Property Information (Bottom Section)
      // Property Usage (Checkboxes)
      const usageY = 343;
      if (formData.propertyUsage === 'Commercial') {
        page1.drawText('X', { x: 256, y: usageY, size, color });
      } else if (formData.propertyUsage === 'Residential') {
        page1.drawText('X', { x: 369, y: usageY, size, color });
      } else if (formData.propertyUsage === 'Industrial') {
        page1.drawText('X', { x: 149, y: usageY, size, color });
      }

      // Row 2: Plot No (L) | Makani No (R)
      page1.drawText(formData.plotNo || '', { x: 140, y: 324, size, color });
      // Row 3: Building Name (L) | Property No (R)
      page1.drawText(formData.buildingName || '', { x: 100, y: 300, size, color });
      page1.drawText(formData.propertyNo || '', { x: 400, y: 300, size, color });
      // Row 4: Property Type (L) | Property Area (R)
      page1.drawText(formData.propertyType || '', { x: 140, y: 275, size, color });
      page1.drawText(formData.propertySize || '', { x: 400, y: 275, size, color });
      // Row 5: Location (L) | DEWA (R)
      page1.drawText(formData.locationArea || '', { x: 100, y: 255, size, color });
      page1.drawText(formData.dewaPremise || '', { x: 400, y: 255, size, color });

      // Contract Terms (Bottom of Page 1)
      page1.drawText(formData.contractFrom || '', { x: 105, y: 195, size, color });
      page1.drawText(formData.contractTo || '', { x: 185, y: 195, size, color });
      page1.drawText(formData.contractValue || '', { x: 450, y: 195, size, color });
      page1.drawText(formData.annualRent || '', { x: 160, y: 170, size, color });
      page1.drawText(formData.securityDeposit || '', { x: 450, y: 170, size, color });
      page1.drawText(formData.paymentMode || '', { x: 180, y: 150, size, color });
  ...
  const renderUploadBox = (id, title, subtitle) => {
    ...
    return (
      <div className={`relative flex flex-col items-center justify-center p-3 sm:p-4 bg-white border-2 border-dashed ${uploadData ? 'border-[#0066FF] bg-blue-50/10' : 'border-slate-200'} rounded-2xl transition-all text-center h-48 sm:h-64 overflow-hidden`}>
        ...
        {uploadData ? (
          <div className="flex flex-col items-center justify-center w-full h-full">
            ...
            <div className="flex flex-col sm:flex-row items-center gap-2 mt-auto w-full">
              <label className="cursor-pointer w-full text-[11px] sm:text-[12px] font-bold text-slate-600 bg-slate-100 hover:bg-slate-200 py-1.5 sm:py-2 rounded-xl transition-colors flex items-center justify-center gap-1 sm:gap-1.5 border border-slate-200 px-1">
                <input type="file" className="hidden" accept="image/*,application/pdf" onChange={(e) => handleFileChange(e, id)} />
                <FileText className="w-3.5 h-3.5 sm:w-4 sm:h-4" /> File
              </label>
              <button
                onClick={() => startCamera(id)}
                className="w-full text-[11px] sm:text-[12px] font-bold text-[#0066FF] bg-blue-50 hover:bg-blue-100 py-1.5 sm:py-2 rounded-xl transition-colors flex items-center justify-center gap-1 sm:gap-1.5 border border-blue-100 px-1"
              >
                <Camera className="w-3.5 h-3.5 sm:w-4 sm:h-4" /> Camera
              </button>
            </div>
          </div>
        ) : (
          <div className="flex flex-col items-center justify-center w-full h-full">
            ...
            <h3 className="font-bold text-[#0B132B] text-[13px] sm:text-base mb-0.5 sm:mb-1">{title}</h3>
            <p className="text-[10px] sm:text-[12px] text-slate-500 mb-3 sm:mb-4 px-1">{subtitle}</p>

            <div className="flex flex-col xl:flex-row items-center gap-1.5 sm:gap-2 mt-auto w-full">
              <label className="cursor-pointer w-full text-[11px] sm:text-[12px] font-bold text-slate-600 bg-slate-100 hover:bg-slate-200 py-1.5 sm:py-2.5 rounded-xl transition-colors flex items-center justify-center gap-1 sm:gap-1.5 border border-slate-200 px-1">
                <input type="file" className="hidden" accept="image/*,application/pdf" onChange={(e) => handleFileChange(e, id)} />
                <FileText className="w-3.5 h-3.5 sm:w-4 sm:h-4" /> <span className="hidden sm:inline">Choose</span> File
              </label>
              <button
                onClick={() => startCamera(id)}
                className="w-full text-[11px] sm:text-[12px] font-bold text-[#0066FF] bg-blue-50 hover:bg-blue-100 py-1.5 sm:py-2.5 rounded-xl transition-colors flex items-center justify-center gap-1 sm:gap-1.5 border border-blue-100 px-1"
              >
                <Camera className="w-3.5 h-3.5 sm:w-4 sm:h-4" /> Camera
              </button>
            </div>
  ...
  return (
    <div ref={wizardRef} className="container mx-auto px-4 pb-12 pt-12 max-w-4xl flex-1 flex flex-col relative scroll-mt-6">
      ...
      {step === 1 && (
        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4">
          <div className="text-center max-w-2xl mx-auto mb-6 sm:mb-8">
            <h1 className="text-[22px] sm:text-3xl font-bold text-[#0B132B] mb-1 sm:mb-3 whitespace-nowrap">Upload your documents</h1>
            <p className="text-slate-500 text-[11px] sm:text-[16px] whitespace-nowrap">
              AI extracts all details automatically. Or skip and fill manually.
            </p>
          </div>

          <div className="grid grid-cols-2 md:grid-cols-3 gap-3 sm:gap-6">
            {renderUploadBox("landlordId", "Landlord ID", "Passport/Emirates ID")}
            {renderUploadBox("tenantId", "Tenant ID", "Passport/Emirates ID")}
            <div className="col-span-1">
              {renderUploadBox("titleDeed", "Title Deed", "Official DLD Document")}
            </div>

            {/* Mobile-only "Fill manually" card to fit perfectly next to Title Deed */}
            <div className="col-span-1 md:hidden flex">
              <button
                onClick={() => setStep(2)}
                className="group relative bg-gradient-to-br from-white to-[#F0F5FF] border border-[#D1E0FF] text-[#0B132B] p-3 rounded-2xl hover:border-[#0066FF] hover:shadow-[0_4px_16px_rgba(0,102,255,0.15)] transition-all w-full h-full flex flex-col items-center justify-center text-center text-[12px] sm:text-[13px] shadow-sm overflow-hidden"
              >
                <div className="w-10 h-10 bg-white shadow-sm border border-[#E5F0FF] text-[#0066FF] rounded-full flex items-center justify-center mb-3 group-hover:scale-110 group-hover:bg-[#0066FF] group-hover:text-white group-hover:border-transparent transition-all duration-300">
                  <Pencil className="w-4 h-4" />
                </div>
                <span className="font-semibold text-slate-600 mb-0.5">No documents?</span>
                <span className="font-bold text-[#0066FF] flex items-center gap-1">
                  Fill manually <ArrowRight className="w-3 h-3 group-hover:translate-x-1 transition-transform" />
                </span>
              </button>
            </div>
          </div>

          <div className="flex flex-col sm:flex-row items-center justify-center mt-8 sm:mt-10 gap-3 sm:gap-4">
            <button
              onClick={handleSimulateExtraction}
              disabled={isExtracting || (!uploads.landlordId && !uploads.tenantId && !uploads.titleDeed)}
              className="bg-[#0066FF] text-white px-4 sm:px-8 py-3 sm:py-4 rounded-xl font-bold hover:bg-blue-700 disabled:bg-slate-300 disabled:cursor-not-allowed shadow-[0_8px_20px_rgba(0,102,255,0.25)] disabled:shadow-none transition-all flex items-center gap-1.5 sm:gap-2 w-full sm:w-auto justify-center text-[13px] sm:text-base whitespace-nowrap"
            >
              {isExtracting ? (
                <>
                ...
                <>
                  Extract Details <span className="hidden sm:inline">Automatically</span> <ArrowRight className="w-4 h-4 sm:w-5 sm:h-5" />
                </>
              )}
            </button>
            <button
              onClick={() => setStep(2)}
              className="hidden md:block bg-white border-2 border-slate-200 text-[#0B132B] px-4 sm:px-8 py-3 sm:py-4 rounded-xl font-bold hover:border-[#0B132B] hover:bg-slate-50 transition-all w-full sm:w-auto text-center text-[13px] sm:text-base whitespace-nowrap"
            >
                ...
                <React.Fragment key={field.name}>
                  ...
                  <div className={`flex flex-col gap-2 ${field.type === 'textarea' || field.fullWidth ? 'col-span-1 sm:col-span-2' : ''}`}>
                    <div className="flex items-center justify-between">
                      <label htmlFor={field.name} className="text-[11px] sm:text-[13px] font-bold text-slate-700">
                        {field.label}
                      </label>
                      {field.checkboxLabel && (
                      ...
                      <div className="flex gap-2 mt-1 mb-1">
                        {field.pills.map(pill => (
                          <button
                            ...
                            className={`px-3 sm:px-4 py-1 sm:py-1.5 text-[11px] sm:text-[12px] font-bold rounded-full border border-slate-200 bg-white transition-colors ${pill === 'Custom' ? 'text-[#0066FF] border-[#0066FF] shadow-[0_2px_8px_rgba(0,102,255,0.15)]' : 'text-slate-600 hover:border-[#0066FF] hover:text-[#0066FF]'}`}>
                            {pill}
                          </button>
                        ))}
                      </div>
                    ...
                    {field.type === 'textarea' ? (
                      <textarea
                        ...
                        className="px-2.5 sm:px-3 py-1.5 rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-[#0066FF] focus:border-transparent min-h-[60px] sm:min-h-[80px] text-slate-700 bg-slate-50 focus:bg-white focus:shadow-sm transition-all text-[10px] sm:text-[13px] font-medium"
                      ...
                      <div className="relative">
                        <select
                          ...
                          value={formData[field.name] || ""}
                          onChange={handleInputChange}
                          className="px-2.5 sm:px-3 py-1.5 w-full rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-[#0066FF] focus:border-transparent text-slate-700 bg-slate-50 focus:bg-white focus:shadow-sm transition-all text-[10px] sm:text-[13px] font-medium appearance-none"
                          required={field.label.includes('*')}
                        >
                        ...
                        <div className="pointer-events-none absolute inset-y-0 right-0 flex items-center px-2.5 sm:px-3 text-slate-500 font-bold text-xs">
                          v
                        </div>
                      </div>
                    ) : field.prefix ? (
                      <div className="flex rounded-xl border border-slate-200 overflow-hidden focus-within:ring-2 focus-within:ring-[#0066FF] focus-within:border-transparent bg-slate-50 focus-within:bg-white focus-within:shadow-sm transition-all">
                        <div className="px-2.5 sm:px-3 py-1.5 border-r border-slate-200 flex items-center justify-center text-[10px] sm:text-[12px] font-bold text-[#0066FF] bg-slate-100/50 whitespace-nowrap shrink-0">
                          {field.prefix}
                        </div>
                        <input
                          type={field.type}
                          ...
                          onChange={handleInputChange}
                          placeholder={field.placeholder || ""}
                          className="px-2.5 sm:px-3 py-1.5 w-full focus:outline-none text-slate-700 bg-transparent text-[10px] sm:text-[13px] font-medium"
                          required={field.label.includes('*')}
                        />
                      </div>
                    ) : field.type === 'date' ? (
                      <input
                        ...
                        value={formData[field.name] || ""}
                        onChange={handleInputChange}
                        className="px-2.5 sm:px-3 py-1.5 rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-[#0066FF] focus:border-transparent text-slate-700 bg-slate-50 focus:bg-white focus:shadow-sm transition-all text-[10px] sm:text-[13px] font-medium"
                        required={field.label.includes('*')}
                      />
                    ) : (
                      <input
